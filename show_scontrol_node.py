...
class show_job():
    ...
    def Check_Single_Job (self,Job_ID):
        try:
            ...
            Job_EndTIme = job_info.split("EndTIme=")[1].split()[0]
            Job_Outfile="N/A"
            Job_Errfile="N/A"
            ...
            if Job_State == "RUNNING":
                x.RUNNING.append(("Runtime: ", Job_Running_time, "Endtime ", Job_EndTIme))
            else:
                x.PENDING.append(("Estimated: ", Job_StartTime, "Reason: ", Job_Reason))
            print(x)
            sys.exit(0)
        except subprocess.CalledProcessError as error:
            print("ShowJob  subprocess command threw an error. {}  error code {}".format( error.returncode, error.output))
